...
def parse_args():
    ...
    parser.add_argument('--reg_rate', nargs = '?', default = '[0.001, 0.001, 0.001, 0.001, 0.1]')
...
class UMF():
    def __init__(self, num_user, src_num_item, dst_num_item, learning_rate = 0.001, reg_rate = [0, 0, 0], epochs = 50, batch_size = 128, num_factor = 50):
    ...
    def build_network(self):
        ...
        self.src_U = tf.Variable(tf.random_normal([self.num_user, self.num_factor], stddev = 0.01), name = 'Src_User')
        self.dst_U = tf.Variable(tf.random_normal([self.num_user, self.num_factor], stddev = 0.01), name = 'Dst_User')
        ...
        self.dst_pred_rating = tf.reduce_sum(tf.multiply(dst_user_latent_factor, dst_item_latent_factor), 1)

        self.loss_Src_Dst_U = tf.nn.l2_loss(self.src_U - self.dst_U)
